...
def load_market_and_news(source: str = "local") -> tuple[dict, dict]:
    if source == "local":
        return load_json(MARKET_PATH), load_json(NEWS_PATH)
    if source == "remote":
        return load_json_from_url(REMOTE_MARKET_URL), load_json_from_url(REMOTE_NEWS_URL)
    if source == "auto":
        try:
            return load_json_from_url(REMOTE_MARKET_URL), load_json_from_url(REMOTE_NEWS_URL)
...
def build_morning_brief(market: dict, news: dict) -> str:
    lines = [
        "早报",
        market.get("date", ""),
        "",
...
def build_evening_brief(market: dict, news: dict) -> str:
    lines = [
        "晚报",
        market.get("date", ""),
        "",
...
def build_brief(mode: str, source: str = "local") -> str:
    market, news = load_market_and_news(source=source)
    if mode == "morning":
        return build_morning_brief(market, news)
    if mode == "evening":
        return build_evening_brief(market, news)
