...
test('keeps Zhejiang-only active storms visible when GDACS is unavailable', async () => {
  const fetchImpl = async url => {
    if (url.includes('geteventlist')) return response({ error: true }, { status: 503 });
    if (url.includes('TyhoonActivity')) {
      return response([{
        tfid: '202612',
        name: '红霞',
        enname: 'NOUL',
  ...
  const result = await getActiveTyphoons({ fetchImpl, now: NOW, zhejiangEnabled: true });
  assert.equal(result.status, 'degraded');
  assert.equal(result.active, true);
  assert.equal(result.storms[0].name.zh, '红霞');
  assert.equal(result.storms[0].classification, 'severe_tropical_storm');
  assert.equal(result.storms[0].intensity.windForceScale, 11);
  assert.equal(result.storms[0].providerIds.zhejiang, '202612');
  assert.match(result.storms[0].detailUrl, /gdacs-tc-202612/);
});
