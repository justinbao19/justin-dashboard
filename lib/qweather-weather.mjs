...
function normalizeAir(payload) {
  ...
  return {
    ...pollutants,
    aqi: { local: number(local.aqi, null), chn: number(local.aqi, null) },
    description: { local: String(local.category || '').trim() || '暂无评级', chn: String(local.category || '').trim() || '暂无评级' },
    standard: String(local.name || '').trim() || '本地空气质量标准',
    primaryPollutant: local.primaryPollutant?.name || null,
    healthEffect: local.health?.effect || null,
    healthAdvice: local.health?.advice?.generalPopulation || null,
    sensitiveAdvice: local.health?.advice?.sensitivePopulation || null,
