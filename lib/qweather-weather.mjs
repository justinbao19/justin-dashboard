...
function localClockMinutes(dateValue) {
...
function isNightAt(dateValue, daily) {
  ...
  const day = (daily || []).find(item => String(item.fxDate || '') === String(dateValue).slice(0, 10)) || daily?.[0];
  const minutes = localClockMinutes(dateValue);
  if (minutes === null) return false;
  if (!day?.sunrise || !day?.sunset) return minutes < 6 * 60 || minutes >= 18 * 60;
...
function uvCategory(index) {
  if (index >= 11) return '极强';
  if (index >= 8) return '很强';
  if (index >= 6) return '较强';
  if (index >= 3) return '中等';
  return '低';
}

function currentUvEstimate(obsTime, today, currentNight) {
  const dailyMax = number(today?.uvIndex, null);
  if (currentNight) return { index: 0, desc: '低', dailyMax, estimated: true };
  if (!Number.isFinite(dailyMax)) return { index: null, desc: null, dailyMax: null, estimated: false };

  const current = localClockMinutes(obsTime);
  const sunrise = localClockMinutes(`${today?.fxDate || ''}T${today?.sunrise || '06:00'}`);
  const sunset = localClockMinutes(`${today?.fxDate || ''}T${today?.sunset || '18:00'}`);
  if (current === null || sunrise === null || sunset === null || sunset <= sunrise) {
    return { index: dailyMax, desc: uvCategory(dailyMax), dailyMax, estimated: false };
  }

  const daylightProgress = Math.max(0, Math.min(1, (current - sunrise) / (sunset - sunrise)));
  const solarFactor = Math.max(0, Math.sin(Math.PI * daylightProgress));
  const index = Math.max(0, Math.round(dailyMax * solarFactor));
  return { index, desc: uvCategory(index), dailyMax, estimated: true };
}
...
export function normalizeQWeatherBundle(bundle, location = {}) {
  ...
  const currentUv = currentUvEstimate(now.obsTime || new Date().toISOString(), today, currentNight);
