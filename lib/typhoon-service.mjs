...
function classificationFromChineseLabel(value, windMs) {
  ...
  if (text.includes('热带扰动') || text.includes('低压区')) return 'tropical_disturbance';
  return classificationFromWind(Number.isFinite(windMs) ? windMs * 3.6 : null);
}

function parseZhejiangWindCircle(value, level) {
  const values = safeText(value, 80).split('|').map(numberOrNull);
  if (values.length !== 4 || values.some(radius => radius === null || radius <= 0)) return null;
  const [northeast, southeast, northwest, southwest] = values;
  return {
    level,
    unit: 'km',
    quadrants: { northeast, southeast, southwest, northwest },
    minRadiusKm: Math.min(...values),
    maxRadiusKm: Math.max(...values)
  };
}

function normalizeZhejiangWindCircles(point) {
  return [
    parseZhejiangWindCircle(point?.radius7, 7),
    parseZhejiangWindCircle(point?.radius10, 10),
    parseZhejiangWindCircle(point?.radius12, 12)
  ].filter(Boolean);
}
...
function normalizeZhejiangPoint(point, { id, kind, sourceId, sourceLabel } = {}) {
  ...
  const rawWindMs = numberOrNull(point?.speed);
  const windMs = rawWindMs !== null && rawWindMs > 0 ? rawWindMs : null;
  const rawWindForceScale = numberOrNull(point?.power);
  const rawPressure = numberOrNull(point?.pressure);
  return {
    id,
    kind,
    sourceId,
    ...
    intensity: {
      ...
      windForceScale: rawWindForceScale !== null && rawWindForceScale > 0 ? rawWindForceScale : windForceScaleFromMs(windMs),
      centralPressureHpa: rawPressure !== null && rawPressure > 0 ? rawPressure : null
    },
    windCircles: normalizeZhejiangWindCircles(point),
    movement: {
...
function stormFromZhejiangEntry(entry, now = new Date()) {
  if (!entry?.point) return null;
  const point = entry.point;
  const tfid = String(entry.tfid);
  return {
    id: `gdacs-tc-${tfid}`,
    providerIds: { gdacs: null, cwa: null, zhejiang: tfid },
    name: {
      display: entry.name.en || entry.name.zh || '未命名热带系统',
      en: entry.name.en || null,
      zh: entry.name.zh || null
    },
    basin: 'WP',
    active: true,
    classification: point.classification,
    ...
    startedAt: null,
    updatedAt: point.validAt,
    stale: !point.validAt || now.getTime() - new Date(point.validAt).getTime() > STALE_AFTER_MS,
    source: {
      provider: '浙江省水利厅',
      upstream: '多机构台风路径汇聚',
      url: 'https://typhoon.slt.zj.gov.cn/',
    ...
    detailUrl: `/typhoon/gdacs-tc-${tfid}`
  };
}
...
export async function getActiveTyphoons({ fetchImpl = fetch, cwaApiKey = '', now = new Date(), zhejiangEnabled = fetchImpl === fetch } = {}) {
  const sources = [];
  let discoveredStorms = [];
  let discoveryError = null;
  try {
    const gdacs = await fetchJson(gdacsSearchUrl(now), { fetchImpl });
    if (!Array.isArray(gdacs?.features)) {
      throw new TyphoonServiceError('GDACS 数据结构异常', { code: 'INVALID_RESPONSE' });
    }
    discoveredStorms = gdacs.features
      .map(feature => normalizeGdacsFeature(feature, now))
      .filter(storm => storm?.active);
    sources.push({
      id: 'gdacs',
      status: discoveredStorms.some(storm => storm.stale) ? 'stale' : 'ok',
      lastUpdatedAt: discoveredStorms.map(storm => storm.updatedAt).filter(Boolean).sort().at(-1) || now.toISOString(),
      message: null
    });
  } catch (error) {
    discoveryError = error instanceof TyphoonServiceError ? error : new TyphoonServiceError('GDACS 数据不可用', { cause: error });
    sources.push({
      id: 'gdacs',
      status: 'error',
      lastUpdatedAt: null,
  ...
  if (!discoveredStorms.length && !zhejiangEntries.length && !sources.some(source => ['ok', 'stale'].includes(source.status))) {
    throw discoveryError || new TyphoonServiceError('台风数据源暂时不可用');
  }

  const matchedZhejiangIds = new Set();
  const zhejiangMergedStorms = discoveredStorms.map(storm => {
    const entry = matchZhejiangEntry(storm, zhejiangEntries);
    if (entry) matchedZhejiangIds.add(entry.tfid);
    return mergeZhejiangCurrent(storm, entry);
  });
  ...
  const gdacsSourceIndex = sources.findIndex(source => source.id === 'gdacs');
  if (gdacsSourceIndex >= 0 && sources[gdacsSourceIndex].status !== 'error') {
    sources[gdacsSourceIndex] = {
      id: 'gdacs',
      status: gdacsStorms.some(storm => storm.stale) ? 'stale' : 'ok',
      lastUpdatedAt: gdacsStorms.map(storm => storm.updatedAt).filter(Boolean).sort().at(-1) || now.toISOString(),
      message: null
    };
  }
  ...
  const zhejiangOnlyStorms = zhejiangEntries
    .filter(entry => !matchedZhejiangIds.has(entry.tfid))
    .map(entry => stormFromZhejiangEntry(entry, now))
    .filter(Boolean);
  const storms = [...mergeStorms(gdacsStorms, cwaStorms), ...zhejiangOnlyStorms]
...
async function getZhejiangDetailById(eventId, zhejiangId, { fetchImpl = fetch } = {}) {
  ...
  const baseStorm = {
    ...
    windCircles: latest.windCircles || [],
    affectedRegions: [],
  ...
  return {
    ...
    mapConfig: {
      ...
      weatherLayers: [
        ...
        { id: 'wind-field', label: '近地面风场', provider: 'Open-Meteo', type: 'model-grid', timeGroup: 'on-demand-model', icon: 'wind', defaultVisible: false },
        { id: 'wave-height', label: '浪高分布', provider: 'Open-Meteo Marine', type: 'model-grid', timeGroup: 'on-demand-model', icon: 'waves', defaultVisible: false }
      ]
    },
    sourceHealth: [
    ...
    attribution: ['浙江省水利厅', 'GDACS', 'CARTO', 'OpenStreetMap', 'Esri', 'RainViewer', 'NASA GIBS', 'Open-Meteo']
  };
}
